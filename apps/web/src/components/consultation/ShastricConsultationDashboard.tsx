...
const BENCHMARK_CHARTS: ChartProfile[] = [
  {
    id: "canonical-modi",
    name: "Narendra Modi",
    dob: "1950-09-17",
...
export function ShastricConsultationDashboard() {
  ...
  const [citySearchText, setCitySearchText] = useState("Vadnagar, Gujarat, India");

  // Consultation Parameters State
  ...
  // Combined searchable profile list
  const allProfiles = [
    ...savedVaultProfiles.map((p) => ({ ...p, tag: p.tag || "Saved Vault" })),
    ...BENCHMARK_CHARTS,
  ];
          ...
          <div className="lg:col-span-4 space-y-4">
            ...
            >
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search celebrity or saved profile..."
                  value={searchQuery}
                  onChange={(e) => {
                  ...
                  >
                    {filteredProfiles.length > 0 ? (
                          ...
                          >
                            ...
                            <span
                              className={`text-[9px] px-1.5 py-0.5 rounded font-bold whitespace-nowrap ${
                                p.tag === "Saved Vault"
                                  ? "bg-cyan-100 dark:bg-cyan-500/15 text-cyan-800 dark:text-cyan-400"
                                  : "bg-amber-100 dark:bg-amber-500/15 text-amber-800 dark:text-amber-400"
                              }`}
                            >
                              {p.tag === "Saved Vault" ? "📂 Vault" : "🌟 Benchmark"}
                            </span>
                          </button>
                        ))}
                      </div>
                      ...
                      <div className="p-4 text-center text-xs text-slate-500">
                        No profiles found. Click <strong>+ New Chart</strong> to create one.
                      </div>
                    )}
                  </div>
                )}
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs font-bold text-cyan-700 dark:text-cyan-400 hover:underline flex items-center gap-1"
                  title="Edit birth parameters in popup modal"
                >
                  <span>✏️</span> Edit
                </button>
              </div>
            ...
            >
              ...
              <div className="border-t pt-3 border-slate-200 dark:border-slate-800">
                <h4 className="text-xs font-semibold mb-2 flex items-center gap-2 text-slate-800 dark:text-slate-300">
                  <Calendar className="w-3.5 h-3.5 text-amber-500" /> Life Scan Horizon
                </h4>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setStartYear(val === "" ? (0 as any) : parseInt(val) || 0);
                      }}
                  ...
                  <div>
                    <label className="block text-xs mb-1 font-semibold text-slate-700 dark:text-slate-300">End Year</label>
                    <input
                      type="number"
                      value={isNaN(endYear) || endYear === 0 ? "" : endYear}
                      onChange={(e) => {
                        const val = e.target.value;
                        setEndYear(val === "" ? (0 as any) : parseInt(val) || 0);
                      }}
                      className="w-full border rounded-lg px-3 py-2 text-sm focus:border-amber-500 focus:outline-none bg-slate-50 dark:bg-slate-950 border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white"
                    />
                  </div>
        ...
        <div className={`${isSidebarCollapsed ? "lg:col-span-12" : "lg:col-span-8"} space-y-6`}>
          ...
          {result && (
            <div className="space-y-6">
              ...
              {/* Sapta-Nadi Weather & Cyclone Alert Banner */}
              {result.sapta_nadi_chakra && (
                <div className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-3 px-4 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs shadow-sm">
                  <div className="flex items-center gap-2">
                    <span className="text-base">🌊</span>
                    <span className="font-semibold text-slate-800 dark:text-slate-300">
                      Sapta-Nadi Dominant: <strong className="text-cyan-700 dark:text-cyan-300">{result.sapta_nadi_chakra.dominant_nadi} Nadi</strong>
                    </span>
                  </div>
                </div>
              )}
