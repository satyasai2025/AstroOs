 ...
 * ranked significators — all pre-computed by the backend KP engine.
 * Verdicts are honest labels (POSITIVE/PARTIAL/WEAK), never a
 * manufactured yes/no.
 */
...
export function KPEventExplorer({ eventPromises }: Props) {
  ...
  const result = eventPromises.find((e) => e.eventKey === eventKey) ?? eventPromises[0];
  const vc = VERDICT_COLORS[result?.promise ?? "WEAK"];
