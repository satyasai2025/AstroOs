...
Restored 2026-07-31 from git history (`8df7a6b^:tests/conftest.py`) — the
"clean up dead weight" commit (8df7a6b) had deleted this file, silently
breaking collection of the whole apps/api/tests suite.
"""
...
async def test_engine():
    ...
    Skips (rather than errors) when TEST_DATABASE_URL is unset — database
    backed tests are opt-in, same intent as the require_test_db() collector.
    """
    if not TEST_DB_URL:
