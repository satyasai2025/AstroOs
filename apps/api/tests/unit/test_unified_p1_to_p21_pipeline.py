"""
Unified P1 -> P21 End-to-End Continuous Pipeline Integration Test

Executes a single continuous execution pipeline proving that all 21 Priorities
...
def test_unified_p1_to_p21_continuous_pipeline():
    """
    Executes a single continuous pipeline asserting every priority from P1 to P21.
    """
    # ── P1: EphemerisWrapper ──────────────────────────────────────────────────
    ...
    exp_container = exp_reg.create_experiment(
        ...
        description="Continuous E2E P1-P21 verification experiment",
    ...
    m250 = data_gov_engine.get_dataset("RS-MARRIAGE-250")
    ...
    bm_astak = data_gov_engine.run_benchmark_suite(BenchmarkSuiteType.BM_ASTAK)
    assert bm_astak.accuracy_score_percent == 100.0
    assert bm_astak.passed_cases_count == 50

    bm_div = data_gov_engine.run_benchmark_suite(BenchmarkSuiteType.BM_DIV)
    assert bm_div.accuracy_score_percent == 100.0
    assert bm_div.passed_cases_count == 75
    print(f"[OK] [P21] ResearchDataGovernanceEngine verified {len(datasets)} datasets (RS-MARRIAGE-250 Clean) and executed 3 benchmark suites (BM_BALA, BM_ASTAK, BM_DIV: 100.0% accuracy).")

    print("\n=======================================================")
    print("ALL P1 -> P21 CONTINUOUS PIPELINE TESTS PASSED 100%!")
    print("=======================================================")
