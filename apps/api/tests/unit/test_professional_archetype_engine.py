...
def test_archetype_specs_integrity():
    """Verify all 12 professional archetypes are properly cataloged with statistical metrics."""
    assert len(ARCHETYPE_SPECS) == 12
    expected_keys = [
        "POLITICIAN_LEADER",
        "ACTOR_CINEMA",
        "SPORTS_ATHLETICS",
...
def test_spiritual_saint_evaluation():
    ...
    saint_planets = {
        ...
        "Sun": {"rashi": "Pisces", "house": 9},
        "Mars": {"rashi": "Gemini", "house": 12},
        "Mercury": {"rashi": "Pisces", "house": 9},
...
def test_tech_ai_engineer_evaluation():
    """Tests Tech / AI Software Architect signature with Mercury in 10th, Rahu in 11th, Mars-Mercury logic axis."""
    tech_planets = {
        "Mercury": {"rashi": "Virgo", "house": 10, "nakshatra": "Chitra"},
        "Rahu": {"rashi": "Aquarius", "house": 11, "nakshatra": "Shatabhisha"},
        "Mars": {"rashi": "Virgo", "house": 10},
        "Jupiter": {"rashi": "Cancer", "house": 8},
        "Saturn": {"rashi": "Libra", "house": 11},
        "Sun": {"rashi": "Leo", "house": 9},
        "Moon": {"rashi": "Taurus", "house": 6},
        "Venus": {"rashi": "Libra", "house": 11},
        "Ketu": {"rashi": "Leo", "house": 5}
    }

    eval_res = ProfessionalArchetypeEngine.evaluate_chart(
        planet_positions=tech_planets,
        lagna_rashi="Sagittarius"
    )

    tech_eval = next(a for a in eval_res.archetype_affinities if a.archetype_key == "TECH_AI_ENGINEER")
    assert tech_eval.affinity_score >= 70.0
    assert any("Mercury" in s["signature_name"] or "Budha" in s["signature_name"] for s in tech_eval.matched_signatures)
    assert any("Rahu" in s["signature_name"] for s in tech_eval.matched_signatures)


def test_doctor_medicine_evaluation():
    """Tests Physician / Surgeon signature with Sun in 6th, Mars in 8th, Jupiter aspecting disease axis."""
    doc_planets = {
        "Sun": {"rashi": "Aries", "house": 6, "nakshatra": "Krittika"},
        "Mars": {"rashi": "Gemini", "house": 8},
        "Jupiter": {"rashi": "Cancer", "house": 9},
        "Moon": {"rashi": "Aries", "house": 6, "nakshatra": "Ashwini"},
        "Saturn": {"rashi": "Aquarius", "house": 4},
        "Mercury": {"rashi": "Taurus", "house": 7},
        "Venus": {"rashi": "Pisces", "house": 5},
        "Rahu": {"rashi": "Capricorn", "house": 3},
        "Ketu": {"rashi": "Cancer", "house": 9}
    }

    eval_res = ProfessionalArchetypeEngine.evaluate_chart(
        planet_positions=doc_planets,
        lagna_rashi="Scorpio"
    )

    doc_eval = next(a for a in eval_res.archetype_affinities if a.archetype_key == "DOCTOR_MEDICINE")
    assert doc_eval.affinity_score >= 70.0
    assert any("Sun" in s["signature_name"] or "Arogya" in s["signature_name"] for s in doc_eval.matched_signatures)
    assert any("Mars" in s["signature_name"] or "Surgical" in s["signature_name"] for s in doc_eval.matched_signatures)
...
def test_d1chart_end_to_end_evaluation():
    ...
    assert len(eval_res.archetype_affinities) == 12
