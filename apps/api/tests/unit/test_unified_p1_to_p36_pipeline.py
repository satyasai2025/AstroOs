"""
AstroOS — Unified Continuous P1 to P36 End-to-End Pipeline Integration Test

Executes the complete research stack continuously from Priority 1 through Priority 36:
  ...
  P36     : Longitudinal Evidence Synthesis & Research Knowledge State Engine
"""
...
from apps.api.domain.research_generalization import GeneralizationVerdict
from apps.api.domain.research_knowledge_state import EvidenceGrade, KnowledgeState
...
def test_unified_p1_to_p36_pipeline_continuous_execution():
    """
    Continuous End-to-End Test for P1 through P36.
    """
    print("\n=======================================================")
    print("STARTING UNIFIED CONTINUOUS P1 -> P36 PIPELINE EXECUTION")
    print("=======================================================")
    ...
    bm_report = benchmark.generate_cross_domain_report("snap-p11-p36-root")
    ...
    recon_res = forensic_engine.reconstruct_research_result("marriage", snapshot_id="snap-p11-p36-root")
    ...
    observation = evidence_reg.register_observation(
        subject_reference="subj-p36-pipeline-01",
        ...
        experiment_id="exp-p36-pipeline",
        p11_snapshot_id="snap-p11-p36-root",
    )
    ...
    validity_engine = ResearchValidityEngine(
        experiment_registry=exp_reg, evidence_registry_engine=evidence_reg,
        cohort_engine=cohort, forensic_engine=forensic_engine,
        publication_engine=pub_engine,
    )
    val_assessment = validity_engine.assess_validity(target_objective="marriage", source_snapshot_id="snap-p11-p36-root")
    ...
    repl_engine = ResearchReplicationEngine(
        experiment_registry=exp_reg, validity_engine=validity_engine,
        evidence_registry_engine=evidence_reg, forensic_engine=forensic_engine,
    )
    ...
    gen_engine = ResearchGeneralizationEngine(
        experiment_registry=exp_reg, replication_engine=repl_engine,
        validity_engine=validity_engine, evidence_registry_engine=evidence_reg,
        forensic_engine=forensic_engine,
    )
    gen_assessment = gen_engine.assess_generalization(target_objective="marriage", source_replication_id=repl_study.replication_id)
    print(f"[OK] [P35] External Validity Assessment completed (Verdict={gen_assessment.overall_verdict.value})")

    # 15. P36 Research Knowledge State Engine
    rks_engine = ResearchKnowledgeStateEngine(
        experiment_registry=exp_reg, generalization_engine=gen_engine,
        replication_engine=repl_engine, validity_engine=validity_engine,
        evidence_registry_engine=evidence_reg, forensic_engine=forensic_engine,
    )
    rks_assessment = rks_engine.synthesize_knowledge_state(target_objective="marriage")
    kRecord = rks_assessment.knowledge_state
    print(f"[OK] [P36] Research Knowledge State synthesized ({kRecord.state_id}, State={kRecord.current_state.value}, Grade={kRecord.evidence_grade.value})")
    print(f"[OK] [P36] Certainty Score computed ({kRecord.certainty_score:.4f})")
    print(f"[OK] [P36] Meta-Analysis computed (Pooled ES={kRecord.meta_analysis.pooled_effect_size:.4f}, I^2={kRecord.meta_analysis.i_squared_heterogeneity:.1f}%)")
    print(f"[OK] [P36] Research Knowledge Snapshot generated (Snapshot ID={rks_assessment.knowledge_snapshot_id})")

    # Pipeline Assertions
    assert rks_assessment.overall_verdict == KnowledgeState.REPLICATED_KNOWLEDGE_STATE
    assert kRecord.evidence_grade == EvidenceGrade.GRADE_A
    assert kRecord.certainty_score >= 0.80
    assert len(rks_assessment.knowledge_state_fingerprint) == 64

    print("\n=======================================================")
    print("ALL P1 -> P36 CONTINUOUS PIPELINE TESTS PASSED 100%!")
    print("=======================================================")
