"""
Unified P1 -> P16 End-to-End Continuous Pipeline Integration Test

Executes a single continuous execution pipeline proving that all 16 Priorities
...
def test_unified_p1_to_p16_continuous_pipeline():
    """
    Executes a single continuous pipeline asserting every priority from P1 to P16.
    """
    # ── P1: EphemerisWrapper ──────────────────────────────────────────────────
    ...
    exp_container = exp_reg.create_experiment(
        ...
        description="Continuous E2E P1-P16 verification experiment",
    ...
    print(f"[OK] [P12] MultiDashaConfluenceEngine generated {len(conf_matrix.confluence_windows)} polymodal timing window(s).")
    ...
    print(f"[OK] [P14] RectificationEngine evaluated {rect_result.total_candidates_evaluated} candidates, best offset: {rect_result.best_candidate.offset_seconds:+d}s.")
    ...
    assert ev_report.top_synergies[0].synergy_multiplier > 1.0
    assert len(ev_report.key_condition_rules) >= 3
    print(f"[OK] [P16] EvidenceIntelligenceEngine dynamically synthesized {ev_report.total_techniques_evaluated} techniques ({ev_report.grade_a_count} Grade-A, {len(ev_report.top_synergies)} Synergies, {len(ev_report.key_condition_rules)} Conditions).")

    print("\n=======================================================")
    print("ALL P1 -> P16 CONTINUOUS PIPELINE TESTS PASSED 100%!")
    print("=======================================================")
