"""
Unified P1 -> P17 End-to-End Continuous Pipeline Integration Test

Executes a single continuous execution pipeline proving that all 17 Priorities
...
def test_unified_p1_to_p17_continuous_pipeline():
    """
    Executes a single continuous pipeline asserting every priority from P1 to P17.
    """
    # ── P1: EphemerisWrapper ──────────────────────────────────────────────────
    ...
    exp_container = exp_reg.create_experiment(
        ...
        description="Continuous E2E P1-P17 verification experiment",
    ...
    # Mathematical attribution verification: sum of contributions = 100%
    ...
    assert all(f.citation_verified is True for f in explanation.atomic_factors if f.classical_citation != "PROVENANCE_NOT_VERIFIED")
    assert len(explanation.counterfactuals) >= 3
    assert len(explanation.provenance_lineage) == 8
    ...
    print("\n=======================================================")
    print("ALL P1 -> P17 CONTINUOUS PIPELINE TESTS PASSED 100%!")
    print("=======================================================")
