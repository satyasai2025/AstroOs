"""
Unified P1 -> P28 End-to-End Continuous Pipeline Integration Test

Executes a single continuous execution pipeline proving that all 28 Priorities
...
def test_unified_continuous_pipeline_p1_to_p28():
    """
    Executes a single continuous execution pipeline proving that all 28 Priorities
    interoperate seamlessly within the AstroOS architecture.
    """
    ...
    exp_container = exp_reg.create_experiment(
        ...
        description="Continuous E2E P1-P28 verification experiment",
    ...
    assert commitment.alpha_spending_method == AlphaSpendingMethod.LAN_DEMETS_OBRIEN_FLEMING
    assert len(commitment.predefined_strata) == 3
    ...
    assert adaptive_report.latest_interim_analysis.reestimated_sample_size >= 300
    assert "ADAPTIVE_RESEARCH_ONLY" in adaptive_report.epistemic_non_causal_statement
    print(f"[OK] [P28] AdaptiveResearchEngine evaluated interim look 1/2 at t={adaptive_report.latest_interim_analysis.information_fraction_t:.2f} (Z={adaptive_report.latest_interim_analysis.observed_interim_z_score:.3f}, Decision: {adaptive_report.latest_interim_analysis.interim_decision.value}, Spending: {commitment.alpha_spending_method.value}, Provenance: {adaptive_report.report_provenance_hash}).")

    print("\n=======================================================")
    print("ALL P1 -> P28 CONTINUOUS PIPELINE TESTS PASSED 100%!")
    print("=======================================================")
