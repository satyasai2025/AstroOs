...
def test_oracle_health_with_d30_and_sbc_vedha():
    """Verify D30 Trimsamsa and SBC Sensitive Tara integration in health domain."""
    # 1. Health with D30 affliction and SBC sensitive tara vedha
    resp = ShastricQAOracle.answer_question(
        question="Mera swasthya aur health kaisa rahega?",
        timeline_windows=MOCK_TIMELINE_WINDOWS,
        d30_data={"operative_lord": "mars", "nature": "malefic"},
        sbc_vedha_data={"hit_sensitive_taras": ["Janma", "Naidhana"]},
    )
    assert resp.is_valid_query
    assert resp.domain == "health"
    assert "D30" in resp.answer_en or "Trimsamsa" in resp.answer_en
    assert "Sarvatobhadra" in resp.answer_en or "SBC" in resp.answer_en
    assert any("D30" in g for g in resp.shastric_rule_grounds)
    assert any("Sarvatobhadra" in g for g in resp.shastric_rule_grounds)

    # 2. Health with D30 protective benefic shield and clear SBC
    resp_shield = ShastricQAOracle.answer_question(
        question="How is my health during this period?",
        timeline_windows=MOCK_TIMELINE_WINDOWS,
        d30_data={"operative_lord": "jupiter", "nature": "benefic"},
        sbc_vedha_data={"hit_sensitive_taras": []},
    )
    assert resp_shield.is_valid_query
    assert "shield" in resp_shield.answer_en.lower() or "protective" in resp_shield.answer_en.lower()


def test_oracle_marriage_with_darakaraka():
    """Verify 7 Chara Karakas (DK) integration in marriage answers."""
    resp = ShastricQAOracle.answer_question(
        question="Meri shaadi aur marriage kab hogi?",
        timeline_windows=MOCK_TIMELINE_WINDOWS,
        chara_karakas={"DK": "venus", "AK": "sun"}
    )
    assert resp.is_valid_query
    assert resp.domain == "marriage"
    assert "Darakaraka" in resp.answer_en or "DK" in resp.answer_en
    assert "Venus" in resp.answer_en
    assert any("Darakaraka" in g for g in resp.shastric_rule_grounds)
