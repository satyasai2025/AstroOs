"""
AstroOS — Verified Cohort Ingestor & Pseudonymization Adapter.

Ingests public historical birth-event research records under the Rodden verification standard:
  AA (kinship data from birth certificate / official registry) → EvidenceTier.T1_DOCUMENT
  A  (quoted biography / verified personal source)             → EvidenceTier.T2_SELF
  B  (biography, unquoted source)                               → EvidenceTier.T2_SELF (flagged)
  C/D/X (unattributed / approximated birth times)               → REJECTED from benchmark
"""
...
def pseudonymize(record_id: str, salt: str = "astroos-salt-2026") -> str:
    """Stable cryptographic pseudonym: hashes raw ID into a private subject_ref."""
    return "S-" + hashlib.sha256(f"{salt}:{record_id}".encode()).hexdigest()[:12]
...
class VerifiedCohortIngestor:
    """
    Ingests Rodden-rated historical research records into AstroOS EventRecord format.
    """
    ...
    def convert(self, rec: dict) -> Optional[EventRecord]:
        ...
        cls_str = rec.get("event_class", "").lower().strip()
        etype = ONTOLOGY_MAP.get((dom_str, cls_str))
        if etype is None:
            return None

        sid = pseudonymize(rec.get("record_id", "rec-0"), self.salt)
        ...
        try:
            return EventRecord(
                ...
                date_precision=prec,
                evidence_tier=tier,
                evidence_refs=[f"public_archive:{rec.get('record_id')}:rod{rating}"],
                severity=rec.get("severity"),
                annotator_ids=["PUBLIC_RESEARCH_CORPUS"],
                cohort_strata={"birth_decade": decade, "source": "public_historical_archive"},
            )
        except Exception:
            return None
