"""
AstroOS — AstroDatabank Adapter

Adapter for the AstroDatabank.xlsx birth chart dataset.
Extends the generic Excel adapter with AstroDatabank-specific
column mappings, metadata, and normalization rules.

Source: AstroDatabank.xlsx (57,466 birth records, 15 columns)
"""
...
class AstroDatabankAdapter(ExcelAdapter):
    """Adapter for the AstroDatabank birth chart dataset."""

    DATASET_ID = "ASTRO-RS-COHORT-v0.1.0"
    DATASET_NAME = "AstroDatabank Birth Chart Cohort"
    SHEET_NAME = "Astro Databank"

    def __init__(self):
        super().__init__(sheet_name=self.SHEET_NAME)

    def get_adapter_name(self) -> str:
        return "AstroDatabank"

    def get_column_mappings(self) -> List[ColumnMapping]:
        """Return the column mappings from AstroDatabank columns to AstroOS fields."""
        return [
            ColumnMapping(
                source_column="fname",
    ...
    def get_source_metadata(self, file_path: str) -> Dict[str, Any]:
        ...
        base_meta.update({
            "name": "AstroDatabank Birth Chart Cohort",
            "description": "Birth chart records extracted from the AstroDatabank database with 15 fields per record.",
            "category": "Research",
            "category_code": "RS",
            "type": "Cohort",
            "type_code": "COHORT",
            ...
            "source_description": "AstroDatabank birth chart database",
            "source_uris": [],
    ...
    def get_normalization_rules(self) -> list:
        """Return normalization rules for AstroDatabank data."""
        return [
            assemble_datetime(
