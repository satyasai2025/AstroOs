...
class ShastricStorySynthesizer:
    ...
    def synthesize_story(
        ...
        # Find the window currently active today
        current_window = next(
            (w for w in timeline_windows if (w.get("window_start") or "")[:10] <= today_iso <= (w.get("window_end") or "")[:10]),
            None
        )
        if not current_window:
            # Fallback to the first future window, or first window
            future_pool = [w for w in timeline_windows if (w.get("window_end") or "")[:10] >= today_iso]
            current_window = future_pool[0] if future_pool else (timeline_windows[0] if timeline_windows else {})

        # 1. Identify Golden Periods in the FUTURE from today onwards
        future_windows = [w for w in timeline_windows if (w.get("window_end") or "")[:10] >= today_iso]
        candidate_pool = future_windows if future_windows else timeline_windows

        golden_windows = [
            w for w in candidate_pool if w.get("decision_tier") == "PRATYAKSHA_PHALA"
        ]
        if not golden_windows and candidate_pool:
            golden_windows = sorted(
                candidate_pool,
                key=lambda w: w.get("probability", w.get("confluence_score", 0.0)),
                reverse=True,
            )
