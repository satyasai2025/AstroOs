...
def run():
    ...
    for ev in KNOWN_EVENTS:
        ...
        tier_icon = {"PRATYAKSHA_PHALA": "🔴", "SUSHUPTA_BEEJA": "🟡", "ALPA_PHALA": "🟠", "SAMANYA_KAL": "⚪"}.get(tier, "❓")
        print(f"  {ev['date']:12} {ev['event'][:52]:52} {dasha_str:20} {tier_icon} {tier}")
    ...
    for w in forward.windows:
        tier_icon = {"PRATYAKSHA_PHALA": "🔴", "SUSHUPTA_BEEJA": "🟡", "ALPA_PHALA": "🟠", "SAMANYA_KAL": "⚪"}.get(w.decision_tier, "❓")
        ws = w.window_start if isinstance(w.window_start, date) else w.window_start.date()
        we = w.window_end   if isinstance(w.window_end,   date) else w.window_end.date()
        print(f"\n  {tier_icon} [{ws} → {we}]")
        ...
        print(f"     Verdict : {w.actionable_verdict}")
        print(f"     [HI]    : {w.explanation_hi}")
