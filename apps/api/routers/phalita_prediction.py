    ...
    try:
        ...
        for w in timeline.windows:
            ...
            mapped_timeline_windows.append({
                ...
                "pratyantardasha": w.pratyantardasha_lord,
                "d10_dignity_summary": w.d10_dignity_summary,
                "bhavachalita_note": w.bhavachalita_note,
                "actionable_verdict": w.actionable_verdict,
...
def ask_shastric_oracle(req: OracleQARequest) -> Dict[str, Any]:
    ...
    try:
        ...
        has_future_windows = any((w.get("window_end") or "")[:10] > today_iso for w in windows)

        # If birth details provided and no future windows available, compute future timeline on the fly
        if (not windows or not has_future_windows) and req.birth_date_iso and req.latitude is not None and req.longitude is not None:
            birth_dt = datetime.fromisoformat(req.birth_date_iso.replace("Z", "+00:00"))
            ...
            timeline = decision_engine.scan_life_timeline(
                ...
                scan_start_year=today_year,
                scan_end_year=today_year + 3,
        ...
        resp = ShastricQAOracle.answer_question(
            ...
            native_name=req.native_name or "Native",
            lang=req.lang or "en",
            d30_data=req.d30_data,
            sbc_vedha_data=req.sbc_vedha_data,
            chara_karakas=req.chara_karakas,
        )
        return asdict(resp)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc))
