# RS-COHORT Canonical Mapping Specification
...
|---|-------------|------|-------------|---------|
| 1 | DocID | Integer | Record identifier | 4260 |
...
| 12 | RR | String (1-2 chars) | Birth time accuracy classification | "AA" |
...
|-------------|---------------|----------------|-------|
| DocID | `_record_id` | `ASTRO-REC-COHORT-{DocID}` | Prefix with cohort source code |
...
| Last Name + First Name | `_person.person_name_display` | Concatenate (or coded ID if First Name empty) | Handle coded identifiers, not names |
...
| — | `_source.primary_source.type` | `cohort_import` | Source type |
| — | `_source.primary_source.attribution` | "RS-COHORT curated dataset" | Attribution |
...
| — | `_privacy.record_privacy_tier` | `public` (all records) | Public dataset |
...
| 9 | Event category classification success rate ≥80% | Validation script | ⏳ |
...
## 9. Variant Specification

### GC-MASTER Variant

When this mapping is applied to GC-MASTER reference charts, use the following overrides:

- `_source.primary_source.type` → `gc_master`
- `_confidence.overall_tier` → `verified` (all GC-MASTER data is reference-grade)
- **No Rodden Rating mapping needed** — GC-MASTER is synthetic/verified reference data

---

## 10. Integration Protocol with Engineering Office

### When Import Pipeline Runs
...
- Source: cohort source file via ER-001 pipeline
- Result: PASS / FAIL
