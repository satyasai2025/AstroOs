#!/usr/bin/env python3
"""
AstroOS Phase II Pipeline Dashboard Generator

Reads tasks_phase2_data.json and generates a self-contained HTML dashboard
with real-time progress tracking and dependency visualization.

Usage:
    python generate_dashboard_phase2.py

Output:
    arundhati_pipeline_progress.html
"""
...
# Reuse shared logic - copy of generate_dashboard.py but with Phase II data
...
def load_data():
    """Load Phase II task data from JSON file."""
    data_path = Path(__file__).parent / 'tasks_phase2_data.json'
...
def generate_html(data):
    ...
    # Success criteria (M2)
    success_criteria = data.get('success_criteria', {}).get('M2', [])
    criteria_html = ''
    if success_criteria:
        criteria_html = '<div class="m2"><strong>M2 Success:</strong><ul>' + ''.join(f'<li>{c}</li>' for c in success_criteria) + '</ul></div>'
...
<div class="footer">
  <div>Scheduled: <strong>astroos-phase-ii-orchestrator-v2</strong> · Auto-updates on refresh</div>
  <div class="source">Source: {meta['source']}</div>
  <div class="generated">Generated {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>
  <div class="created-by">Created with generate_dashboard_phase2.py — local, offline, reproducible</div>
</div>

<script>
const saved = JSON.parse(localStorage.getItem('arundhati_progress') || '{{}}');
...
document.addEventListener('click', e => {{
  ...
  localStorage.setItem('arundhati_progress', JSON.stringify(
...
def main():
    ...
    out_path = Path(__file__).parent / 'arundhati_pipeline_progress.html'
    with open(out_path, 'w') as f:
        f.write(html)
    print(f'✓ Phase II dashboard generated: {out_path}')
