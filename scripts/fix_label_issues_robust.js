...
for (const file of files) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Check if line strictly begins with an opening <input or <textarea tag
    const startMatch = line.match(/^(\s*<)(input|textarea)\b/i);
    if (startMatch) {
      const tag = startMatch[2];

      // Gather multi-line tag context to inspect existing attributes
      let tagFullContext = '';
      ...
      // Replace ONLY the opening tag at the very start of line i
      const prefix = startMatch[1];
      lines[i] = line.replace(/^(\s*<)(input|textarea)\b/i, `${prefix}${tag} aria-label="${labelText}"`);
      modified = true;
      fixedCount++;
    }
  }
...
console.log(`\nRobustly updated ${fixedCount} input/textarea tags with aria-label!`);
